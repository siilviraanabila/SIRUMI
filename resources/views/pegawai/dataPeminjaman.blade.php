...
<body style="background-color: #D3F0FF;"> 
    ...
    <section id="home" class="py-5">
        <div style="margin-left: 16rem;">
            <div class="col-span-4 text-center mb-8">
                <h5 class="font-bold text-black" style="font-size: 30px;">Daftar Peminjaman</h5>
            </div>
                                ...
                                <tr id="deleteForm{{ $item->peminjaman_id }}" class="list border-b bg-white border-gray-700 hover:bg-gray-60 hover:bg-gray-200"> 
                                    ...
                                    <td class="px-4 py-2">
                                        <div class="flex justify-end space-x-4">
                                            <a href="{{ route('showdataPeminjaman', ['id' => $item->peminjaman_id]) }}" class="text-violet-500">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                                </svg>
                                            </a>
                                            <a href="{{ route('editdataPeminjaman', ['id' => $item->peminjaman_id]) }}" class="text-cyan-500">
                                            ...
                                            <a href="javascript:void(0)" onclick="deletePeminjaman('{{ $item->peminjaman_id }}')" class="text-red-500">
                    ...
                    <script>
                        ...
                        function deletePeminjaman(id) {
                            if (confirm('Anda yakin ingin menghapus peminjaman ini?')) {
                                $.ajax({
                                    type: 'DELETE',
                                    url: '/pegawai/deletePeminjaman/' + id,
                                    data: {
                                        "_token": "{{ csrf_token() }}",
                                        "_method": "DELETE"
                                    },
